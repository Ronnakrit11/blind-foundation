...
export const users = pgTable("users", {
	...
	// LINE Login fields
	lineId: varchar("line_id", { length: 100 }).unique(),
	lineAccessToken: text("line_access_token"),
	lineRefreshToken: text("line_refresh_token"),
	lineTokenExpiresAt: timestamp("line_token_expires_at"),
...
// export const paymentTransactions = pgTable("verified_slips", {
// 	id: serial("id").primaryKey(),
...
export const paymentSettings = pgTable("payment_settings", {
	id: serial("id").primaryKey(),
	name: varchar("name", { length: 100 }).notNull(),
	ibId: text("ibId ").notNull().default(""),
...
export const paymentTransactionsRelations = relations(
	...
	({ one }) => ({
		...
		project: one(templeProjects, {
			fields: [paymentTransactions.projectId],
			references: [templeProjects.id],
		}),
	})
);
...
export type NewGoldProduct = Omit<GoldProduct, "id">;

export type DepositLimit = InferSelectModel<typeof depositLimits>;
export type NewDepositLimit = Omit<DepositLimit, "id">;

// Banners table for storing homepage banners
...
// Temple News table for storing news articles
...
// Temple Projects table for storing temple donation projects
export const templeProjects = pgTable("temple_projects", {
	id: serial("id").primaryKey(),
	title: varchar("title", { length: 255 }).notNull(),
	slug: varchar("slug", { length: 255 }).notNull().unique(),
	description: text("description"),
...
export const templeProjectsRelations = relations(templeProjects, ({ one }) => ({
	author: one(users, {
		fields: [templeProjects.createdBy],
		references: [users.id],
	}),
}));
...
export type TempleNews = InferSelectModel<typeof templeNews>;
export type NewTempleNews = Omit<TempleNews, "id" | "createdAt" | "updatedAt">;
